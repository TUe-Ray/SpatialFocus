#!/bin/bash
#SBATCH --job-name=Eval_no_spatial_25p
#SBATCH --nodes=1
...
# ============================================================
# NOTE: Eval for the no-spatial ablation checkpoint.
# The trained checkpoint has NO spatial tower / fusion block —
# only LoRA + mm_projector on top of SigLIP + LLaVA-Next-Video.
# Set PRETRAINED_LOCAL to the output dir of train_vsi_no_spatial_ablation.sh.
# ============================================================
NOTE="Eval no-spatial ablation (pure SigLIP+LoRA, 25% data) on VSI-Bench"
echo "-------- Note --------"
echo "  note: $NOTE"

# *** UPDATE THIS to the actual checkpoint directory after training ***
PRETRAINED_LOCAL="${PRETRAINED_LOCAL:-/leonardo_work/EUHPC_D32_006/Train_Model/VLM3R/ablation_no_spatial_25p_JOBID}"

RUN_NAME="${RUN_NAME:-${SLURM_JOB_NAME:-Eval_no_spatial_25p}}"
...
# No spatial tower args needed — the checkpoint config has none.
...
echo "Running Leonardo offline evaluation (no-spatial ablation)"
