...
CONDA_ENV_NAME="${CONDA_ENV_NAME:-vlm3rEOMT}"
# ============================================================
# User-defined variables: EoMT round-1 comparison
# ============================================================
# Supported modes from configs/eomt/eomt_selective_3d_round1.json:
# - baseline
# - selective_soft
...
CUT3R_ROOT_OVERRIDE="${CUT3R_ROOT_OVERRIDE:-/leonardo/home/userexternal/shuang00/VLM-3R_SN/CUT3R}"
...
REQUIRE_SPATIAL_FEATURES="True"     # CUT3R+EoMT training expects pre-extracted .pt token pairs
SEED="${SEED:-42}"
...
export CUT3R_ROOT_OVERRIDE="$CUT3R_ROOT_OVERRIDE"
...
# ============================================================
RUN_NAME="${RUN_NAME_OVERRIDE:-$SUFFIX}"
if [[ -n "${OUTPUT_DIR_OVERRIDE:-}" ]]; then
    OUTPUT_DIR="$OUTPUT_DIR_OVERRIDE"
    MID_RUN_NAME="${MID_RUN_NAME_OVERRIDE:-$(basename "$OUTPUT_DIR")}"
else
    MID_RUN_NAME="${MID_RUN_NAME_OVERRIDE:-$SUFFIX}"
...
if [[ ! -f "$EOMT_EXPERIMENT_CONFIG_PATH" ]]; then
    echo "[ERROR] EoMT experiment config not found: $EOMT_EXPERIMENT_CONFIG_PATH"
    exit 1
fi
if [[ ! -f "$EOMT_CONFIG_PATH" ]]; then
    echo "[ERROR] EoMT config YAML not found: $EOMT_CONFIG_PATH"
    exit 1
fi
if [[ -z "$EOMT_CKPT_PATH" ]]; then
    echo "[ERROR] EOMT_CKPT_PATH is empty. Export it or set it in this script."
    exit 1
fi
if [[ ! -f "$EOMT_CKPT_PATH" ]]; then
...
if [[ ! -d "$CUT3R_ROOT_OVERRIDE" ]]; then
    echo "[ERROR] CUT3R root not found: $CUT3R_ROOT_OVERRIDE"
    exit 1
fi

echo "[LOCAL MODEL] model_name_or_path=$LOCAL_MODEL_BASE"
echo "[LOCAL MODEL] vision_tower=$LOCAL_SIGLIP"
echo "[CUT3R] root=$CUT3R_ROOT_OVERRIDE"
echo "[CUT3R] spatial_features_root=$SPATIAL_FEATURES_ROOT"
echo "[CUT3R] spatial_features_subdir=$SPATIAL_FEATURES_SUBDIR"
echo "[CUT3R] require_spatial_features=$REQUIRE_SPATIAL_FEATURES"
...
case "$MODEL_FUSION_BLOCK" in
    cross_attention)
        echo "[FUSION] cross_attention: original CUT3R method (Q=2D, KV selected by MODEL_SPATIAL_TOWER_SELECT_FEATURE=$MODEL_SPATIAL_TOWER_SELECT_FEATURE)"
        ;;
    svf_patch_only)
...
for mode_name in "${VALID_EOMT_MODES[@]}"; do
    if [[ "$EOMT_EXPERIMENT_MODE" == "$mode_name" ]]; then
        IS_VALID_EOMT_MODE="True"
        break
    fi
done

if [[ "$IS_VALID_EOMT_MODE" != "True" ]]; then
    echo "[ERROR] Unsupported EOMT_EXPERIMENT_MODE: $EOMT_EXPERIMENT_MODE"
    echo "[ERROR] Supported values: ${VALID_EOMT_MODES[*]}"
    exit 1
fi

echo "[EOMT] comparison family: eomt_selective_3d_round1"
echo "[EOMT] selected comparison mode: $EOMT_EXPERIMENT_MODE"
...
declare -A TRAINING_ARGS=(
    ...
    [save_steps]="$SAVE_STEPS"  # Save every 50 steps by default; override if needed
...
echo "  SEED:                               $SEED"
if [[ "$RESUME_CHECKPOINT_PATH" != "none" ]]; then
    echo "  *** RESUMING TRAINING — state will be loaded from checkpoint ***"
else
